...
import user.model.{Group, Viewer, ViewerRole}

/**
 * Model class representing the viewer and editor group relations of a target asset.
 ...
 */
case class AssetViewerCombinator(viewers: Set[Group], editors: Set[Group], maintainers: Set[Group])

object AssetViewerCombinator {

  def fromRelations(relations: Seq[(Viewer, Group)]): AssetViewerCombinator = {
      var maintainers: Set[Group] = Set()
      var editors: Set[Group] = Set()
      var viewers: Set[Group] = Set()
      relations.foreach(rel => {
        val (viewer, group) = rel
      ...
      AssetViewerCombinator(viewers, editors, maintainers)
  }
