...
import modules.asset.model.AssetConstraint
...
trait ConstraintProcessor {
   ...
   */
  def preprocessConstraint(constraint: AssetConstraint): AssetConstraint = {
    ...
    AssetConstraint(constraint.id, c, v1, v2, constraint.typeId)
  }
   ...
   */
  def isValidConstraint(constraint: AssetConstraint): Status = {
    constraint.c match {
   ...
   * @return result
   */
  def hasMatchingProperties(constraints: Seq[AssetConstraint]): Boolean = {
   ...
   * @return result
   */
  def hasNoDuplicates(constraints: Seq[AssetConstraint]): Boolean = {
